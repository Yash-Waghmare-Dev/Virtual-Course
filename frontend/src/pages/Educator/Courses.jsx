       ...
       <div className='w-[100%] min-h-screen p-4 sm:p-6 bg-gray-100'>
        ...
        <div className='hidden md:block bg-white rounded-xl shadow p-4 overflow-x-auto'>
            <table className='min-w-full text-sm'>
                ...
                <tbody> 
                    {
                        creatorCourseData?.data?.map((course, index)=>(
                            
                    <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
                        <td className='py-3 px-4 flex items-center gap-4'>
                            {course?.thumbnail ? <img src={course?.thumbnail} className='w-25 h-14 object-cover rounded-md' name='thumbnail'  alt="" /> :  <img src={img} className='w-25 h-14 object-cover rounded-md' name='thumbnail'  alt="" />}
                            <span className=''>{course.title}</span>
                        </td>
                        {course?.price ? <td className='px-4 py-3'>₹ {course?.price}</td> : <td className='px-4 py-3'>₹ NA</td>}
                        <td className='px-4 py-3'>
                            <span className={`px-3 py-1 rounded-full text-xs bg-red-100 text-red-600 ${course.isPublished ? "bg-green-100 text-green-600 ":"bg-red-100 text-red-600"}`}>{course.isPublished ? "Published":'Draft'}</span>
                        </td>
                        <td className='px-4 py-3'>
        ...
        <div className='md:hidden space-y-4'>
            ...
            <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
                <div className='flex gap-4 items-center'>
                    {course?.thumbnail ? <img src={course?.thumbnail} className='w-16 h-16 rounded-md object-cover' alt="" /> : <img src={img} name='thumbnail' className='w-16 h-16 rounded-md object-cover' alt="" />}

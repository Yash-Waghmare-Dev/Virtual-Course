...
const AllCourse = () => { 
  ...
  const toggleCategory = (e) => {
    if(category.includes(e.target.value) ){
      setCategory(prev=>prev.filter(category => category !== e.target.value))
    }
    else{
  ...
  return (
    <div className='flex min-h-screen bg-gray-50'>
      <Nav/>
        <button className='fixed top-20 left-4 z-100 bg-white text-black px-3 py-1 rounded md:hidden border-2 border-black' onClick={()=>setIsSidebarVisible(prev=>!prev)}>
          {
